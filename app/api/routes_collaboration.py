...
async def create_comment(draft_id: int, req: CommentRequest, request: Request):
...
async def list_comments(draft_id: int, request: Request):
    require_permission(request, "approval:write")
    tenant_id = resolve_tenant_id(getattr(request.state, "tenant_id", None))
    return await get_comments(draft_id=draft_id, tenant_id=tenant_id)


@router.post("/drafts/{draft_id}/assign")
async def assign(draft_id: int, req: AssignRequest, request: Request):
    require_permission(request, "approval:write")
    tenant_id = resolve_tenant_id(getattr(request.state, "tenant_id", None))
    return await assign_draft(
        draft_id=draft_id,
...
async def unassign(draft_id: int, request: Request):
    require_permission(request, "approval:write")
    tenant_id = resolve_tenant_id(getattr(request.state, "tenant_id", None))
    actor = request.headers.get("X-Actor", "system")
    return await unassign_draft(
        draft_id=draft_id,
        unassigned_by=actor,
        tenant_id=tenant_id,
    )
...
async def get_assigned(accountant: str, request: Request, status: str = None):
    require_permission(request, "approval:write")
    tenant_id = resolve_tenant_id(getattr(request.state, "tenant_id", None))
    return await get_assigned_drafts(
