...
def get_kpi(request: Request):
    ...
    cur.execute(
        "SELECT COALESCE(SUM(amount),0) as total_amount FROM bank_transactions WHERE tenant_id = %s",
        (tenant_id,),
    )
    total_amount = float(cur.fetchone()["total_amount"])
    cur.execute(
        "SELECT COALESCE(SUM(amount),0) as total FROM bank_transactions WHERE tenant_id = %s AND amount > 0",
        (tenant_id,),
    )
    total_inflow = float(cur.fetchone()["total"])
    cur.execute(
        "SELECT COALESCE(SUM(ABS(amount)),0) as total FROM bank_transactions WHERE tenant_id = %s AND amount < 0",
        (tenant_id,),
    )
...
@router.get("/cashflow")
def get_cashflow(request: Request):
    tenant_id = getattr(request.state, "tenant_id", "default")
    conn = get_db()
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute("""
...
def finance_summary(request: Request):
    ...
    # pipeline_runs, journal_entries, coa are global tables (no tenant_id column)
    cur.execute("SELECT COUNT(*) as total FROM pipeline_runs")
