from datetime import datetime, timezone

from app.api.db import get_conn, _q


def _norm(value):
    return " ".join(str(value or "").strip().lower().split())


def _days_since(last_used_at):
    if not last_used_at:
        return None
    if isinstance(last_used_at, str):
        try:
            last_used_at = datetime.fromisoformat(last_used_at.replace("Z", "+00:00"))
        except Exception:
            return None
    if last_used_at.tzinfo is None:
        last_used_at = last_used_at.replace(tzinfo=timezone.utc)
    return max(0, (datetime.now(timezone.utc) - last_used_at).days)


async def save_transaction_memory(description, partner, amount, account_code, tenant_id: str = "default"):
    desc = _norm(description)
    part = _norm(partner)
    if not account_code:
        return {"ok": False, "error": "account_code is required"}

    async with get_conn() as conn:
        existing = await conn.fetchrow(_q("""
            SELECT id, usage_count FROM transaction_memory
            WHERE tenant_id = %s
              AND COALESCE(description, '') = %s
              AND COALESCE(partner, '') = %s
              AND account_code = %s
            LIMIT 1
        """), tenant_id, desc, part, account_code)

        if existing:
            row = await conn.fetchrow(_q("""
                UPDATE transaction_memory
                SET usage_count = usage_count + 1,
                    last_used_at = NOW(), updated_at = NOW(), amount = %s
                WHERE id = %s AND tenant_id = %s
                RETURNING id, usage_count
            """), amount, existing["id"], tenant_id)
            return {"ok": True, "mode": "updated", "tenant_id": tenant_id,
                    "id": row["id"], "usage_count": row["usage_count"]}

        row = await conn.fetchrow(_q("""
            INSERT INTO transaction_memory
                (tenant_id, description, partner, amount, account_code,
                 confidence, usage_count, last_used_at, created_at, updated_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, NOW(), NOW(), NOW())
            RETURNING id, usage_count
        """), tenant_id, desc, part, amount, account_code, 1.0, 1)
        return {"ok": True, "mode": "inserted", "tenant_id": tenant_id,
                "id": row["id"], "usage_count": row["usage_count"]}


async def find_memory_match(description, partner, tenant_id: str = "default"):
    desc = _norm(description)
    part = _norm(partner)
    if not desc and not part:
        return None

    async with get_conn() as conn:
        row = await conn.fetchrow(_q("""
            SELECT id, description, partner, account_code, usage_count, last_used_at
            FROM transaction_memory
            WHERE tenant_id = %s
              AND (
                    (%s <> '' AND COALESCE(description, '') = %s)
                 OR (%s <> '' AND COALESCE(partner, '') = %s)
              )
            ORDER BY
                CASE
                    WHEN %s <> '' AND %s <> ''
                         AND COALESCE(description, '') = %s
                         AND COALESCE(partner, '') = %s THEN 0
                    WHEN %s <> '' AND COALESCE(description, '') = %s THEN 1
                    WHEN %s <> '' AND COALESCE(partner, '') = %s THEN 2
                    ELSE 3
                END,
                usage_count DESC, id DESC
            LIMIT 1
        """), tenant_id,
        desc, desc, part, part,
        desc, part, desc, part,
        desc, desc, part, part)

    if not row:
        return None

    usage_count = int(row["usage_count"] or 1)
    days_since_seen = _days_since(row.get("last_used_at"))

    if desc and part and row.get("description") == desc and row.get("partner") == part:
        match_type = "description_partner_exact"
    elif desc and row.get("description") == desc:
        match_type = "description_exact"
    elif part and row.get("partner") == part:
        match_type = "partner_exact"
    else:
        match_type = "generic"

    confidence = 0.88
    if usage_count >= 5:
        confidence = 0.96
    elif usage_count >= 3:
        confidence = 0.93
    elif usage_count >= 2:
        confidence = 0.90

    if days_since_seen is None:
        confidence -= 0.03
    elif days_since_seen > 45:
        confidence -= 0.08
    elif days_since_seen > 30:
        confidence -= 0.04

    return {
        "tenant_id": tenant_id,
        "account_code": row["account_code"],
        "reason": "memory_match",
        "confidence": round(max(0.50, min(confidence, 0.99)), 2),
        "source": "memory",
        "memory_usage_count": usage_count,
        "memory_description": row["description"],
        "memory_partner": row["partner"],
        "memory_match_type": match_type,
        "memory_days_since_seen": days_since_seen,
    }


async def list_transaction_memory(limit: int = 100, tenant_id: str = "default"):
    async with get_conn() as conn:
        rows = await conn.fetch(_q("""
            SELECT id, tenant_id, description, partner, amount, account_code,
                   confidence, usage_count, last_used_at, created_at, updated_at
            FROM transaction_memory
            WHERE tenant_id = %s ORDER BY id DESC LIMIT %s
        """), tenant_id, limit)
    return [dict(r) for r in rows]
