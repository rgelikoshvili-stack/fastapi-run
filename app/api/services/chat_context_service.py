        ...
        async with get_conn() as conn:
            ...
            if "approval" in intents:
                ...
                try:
                    ...
                    """), tenant_id, exclude)
                    ctx["recent_drafts"] = [_safe_dict(r) for r in rows]
                except Exception as e:
                ...
                try:
                    ...
                    """), tenant_id)
                    ctx["queue"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "bank" in intents:
                try:
                    ...
                    bank_rows = [_safe_dict(r) for r in rows]
                    ctx["bank_accounts"] = bank_rows
                    totals: dict = {}
                    for r in bank_rows:
                        cur_key = r.get("currency", "GEL")
                        totals[cur_key] = totals.get(cur_key, 0.0) + float(r.get("balance") or 0)
                    ctx["bank_summary"] = {"accounts_count": len(bank_rows), "totals": totals}
                except Exception as e:
                ...
                try:
                    ...
                    """), tenant_id)
                    txns = [_safe_dict(r) for r in rows]
            ...
            if "invoices" in intents:
                try:
                    ...
                    """), tenant_id)
                    ctx["invoices"] = [_safe_dict(r) for r in rows]
                except Exception as e:
                ...
                try:
                    ...
                    """), tenant_id)
                    ctx["outgoing_invoices"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "documents" in intents:
                try:
                    rows = await conn.fetch(_q("""
                        ...
                        ORDER BY created_at DESC LIMIT 5
                    """), tenant_id)
                    ctx["documents"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "waybills" in intents:
                try:
                    rows = await conn.fetch(_q("""
                        ...
                        ORDER BY created_at DESC LIMIT 5
                    """), tenant_id)
                    ctx["waybills"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "tax_invoices" in intents:
                try:
                    rows = await conn.fetch(_q("""
                        ...
                        ORDER BY created_at DESC LIMIT 5
                    """), tenant_id)
                    ctx["tax_invoices"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "notifications" in intents:
                try:
                    ...
                    """), tenant_id)
                    ctx["notifications"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "payroll" in intents:
                try:
                    rows = await conn.fetch(_q("""
                        ...
                        ORDER BY created_at DESC LIMIT 5
                    """), tenant_id)
                    ctx["payroll_drafts"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "audit" in intents:
                try:
                    rows = await conn.fetch(_q("""
                        ...
                        ORDER BY created_at DESC LIMIT 10
                    """), tenant_id)
                    ctx["audit_recent"] = [_safe_dict(r) for r in rows]
                except Exception as e:
            ...
            if "decisions" in intents:
                try:
                    ...
                    """), tenant_id)
                    ctx["decisions_pending"] = [_safe_dict(r) for r in rows]
                except Exception as e:
...
def _safe_dict(row) -> dict:
    """Convert asyncpg Record to plain dict with serializable values."""
    d = dict(row)
    for k, v in d.items():
        try:
