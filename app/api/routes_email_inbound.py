...
async def _find_tenant_by_token(token: str) -> str | None:
    try:
...
async def _get_or_create_submit_token(tenant_id: str) -> str:
    """Return existing submit_token or generate and save a new one."""
    try:
        async with get_conn() as conn:
            row = await conn.fetchrow(_q(
                "SELECT submit_token FROM tenants WHERE id = %s"
            ), tenant_id)
...
async def email_inbound_webhook(request: Request):
    ...
    tenant_id = await _find_tenant_by_token(token)
    if not tenant_id:
    ...
    for att in attachments:
        ...
        if await _already_processed(tenant_id, msg_uid, filename):
        ...
        doc_id = await _save_email_document(tenant_id, msg_uid, filename, file_data, raw_text)
...
async def get_submit_address(request: Request):
    ...
    token = await _get_or_create_submit_token(tenant_id)
