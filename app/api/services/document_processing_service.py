...
async def _get_tenant_vat(tenant_id: str) -> bool:
    try:
        async with get_conn() as conn:
            row = await conn.fetchrow(
                _q("SELECT is_vat_payer FROM tenants WHERE tenant_id = %s"), tenant_id
            )
        return bool(row["is_vat_payer"]) if row else True
    except Exception as e:
        log.warning("_get_tenant_vat: %s", e)
        return True


async def _upsert_counterparty(tenant_id: str, inn: str, name: str, cp_type: str) -> None:
    if not inn:
        return
    try:
        async with get_conn() as conn:
...
async def _mark_doc_status(doc_id: int, status: str, tenant_id: str):
    try:
        async with get_conn() as conn:
            await conn.execute(
...
async def _apply_completeness(tenant_id: str, source_doc_id: int, extracted: ExtractedDocument):
    """
    ...
    try:
        ...
        if seller_inn and buyer_inn:
            async with get_conn() as conn:
                ...
                if wb:
                    pipeline_docs.append({"doc_type": "waybill", "doc_number": wb["waybill_number"] or "",
                                          "total_amount": float(wb["total_amount"] or 0),
                                          "seller_inn": wb["seller_inn"], "buyer_inn": wb["buyer_inn"]})

                ti = await conn.fetchrow(_q(
                ...
                if ti:
                    pipeline_docs.append({"doc_type": "tax_invoice", "doc_number": ti["invoice_number"] or "",
                                          "total_amount": float(ti["total_amount"] or 0),
                                          "seller_inn": ti["seller_inn"], "buyer_inn": ti["buyer_inn"]})

                ci = await conn.fetchrow(_q(
                    "SELECT invoice_number, total_amount, seller_inn, buyer_inn "
                    "FROM commercial_invoices WHERE tenant_id=%s AND seller_inn=%s AND buyer_inn=%s "
                    "ORDER BY created_at DESC LIMIT 1"
                ), tenant_id, seller_inn, buyer_inn)
                if ci:
                    pipeline_docs.append({"doc_type": "invoice", "doc_number": ci["invoice_number"] or "",
                                          "total_amount": float(ci["total_amount"] or 0),
                                          "seller_inn": ci["seller_inn"], "buyer_inn": ci["buyer_inn"]})
...
async def _refresh_related_drafts(tenant_id: str, seller_inn: str | None, buyer_inn: str | None):
    """
...
async def _try_match_triangle(tenant_id: str, doc_type: str, doc_id: int, doc: dict):
    """After inserting a doc, find candidates and create/update triangle match."""
    try:
        async with get_conn() as conn:
            candidates = await find_candidates(conn, tenant_id, doc_type, doc)
            if not candidates:
                return None
            ...
            match_id = await upsert_triangle_match(conn, tenant_id, match_data)
...
async def _also_queue_for_pipeline(tenant_id: str, file_bytes: bytes, mime_type: str, filename: str):
...
async def _process_document_background(
    ...
    try:
        ...
        await _apply_completeness(tenant_id, doc_id, extracted)

        await _upsert_counterparty(
            tenant_id,
