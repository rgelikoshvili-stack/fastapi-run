from fastapi import APIRouter, Request
import hashlib, secrets
from datetime import datetime, timedelta
from app.api.db import get_conn, _q

router = APIRouter(prefix="/security", tags=["security"])

_CREATE_RATE_LIMITS = """
    CREATE TABLE IF NOT EXISTS rate_limits (
        id SERIAL PRIMARY KEY,
        ip_address VARCHAR(50),
        endpoint VARCHAR(200),
        request_count INT DEFAULT 1,
        window_start TIMESTAMP DEFAULT NOW(),
        created_at TIMESTAMP DEFAULT NOW()
    )
"""
_CREATE_SECURITY_EVENTS = """
    CREATE TABLE IF NOT EXISTS security_events (
        id SERIAL PRIMARY KEY,
        event_type VARCHAR(50),
        ip_address VARCHAR(50),
        endpoint VARCHAR(200),
        details TEXT,
        severity VARCHAR(20) DEFAULT 'LOW',
        created_at TIMESTAMP DEFAULT NOW()
    )
"""
_CREATE_API_TOKENS = """
    CREATE TABLE IF NOT EXISTS api_tokens (
        id SERIAL PRIMARY KEY,
        token_hash VARCHAR(200) UNIQUE,
        name VARCHAR(100),
        permissions TEXT[],
        is_active BOOLEAN DEFAULT TRUE,
        last_used TIMESTAMP,
        expires_at TIMESTAMP,
        created_at TIMESTAMP DEFAULT NOW()
    )
"""
_CREATE_BLOCKED_IPS = """
    CREATE TABLE IF NOT EXISTS blocked_ips (
        id SERIAL PRIMARY KEY,
        ip_address VARCHAR(50) UNIQUE,
        reason TEXT,
        blocked_at TIMESTAMP DEFAULT NOW(),
        blocked_until TIMESTAMP
    )
"""


async def _ensure_tables(conn):
    await conn.execute(_CREATE_RATE_LIMITS)
    await conn.execute(_CREATE_SECURITY_EVENTS)
    await conn.execute(_CREATE_API_TOKENS)
    await conn.execute(_CREATE_BLOCKED_IPS)


@router.post("/token/generate")
async def generate_token(payload: dict):
    token = secrets.token_hex(32)
    token_hash = hashlib.sha256(token.encode()).hexdigest()
    expires_days = payload.get("expires_days", 30)
    expires_at = datetime.utcnow() + timedelta(days=expires_days)
    async with get_conn() as conn:
        await _ensure_tables(conn)
        tid = await conn.fetchval(_q("""
            INSERT INTO api_tokens (token_hash, name, permissions, expires_at)
            VALUES (%s,%s,%s,%s) RETURNING id
        """), token_hash, payload.get("name", "API Token"),
            payload.get("permissions", ["read"]), expires_at)
    return {"ok": True, "token": token, "token_id": tid,
            "expires_at": expires_at.isoformat(), "note": "Save this token — it won't be shown again"}


@router.post("/token/validate")
async def validate_token(payload: dict):
    token = payload.get("token", "")
    token_hash = hashlib.sha256(token.encode()).hexdigest()
    async with get_conn() as conn:
        await _ensure_tables(conn)
        tok = await conn.fetchrow(_q("""
            SELECT * FROM api_tokens WHERE token_hash=%s AND is_active=TRUE
            AND (expires_at IS NULL OR expires_at > NOW())
        """), token_hash)
        if tok:
            await conn.execute(_q(
                "UPDATE api_tokens SET last_used=NOW() WHERE token_hash=%s"
            ), token_hash)
    if not tok:
        return {"ok": False, "valid": False, "message": "Invalid or expired token"}
    return {"ok": True, "valid": True, "name": tok["name"],
            "permissions": tok["permissions"], "expires_at": str(tok["expires_at"])}


@router.get("/tokens")
async def list_tokens():
    async with get_conn() as conn:
        await _ensure_tables(conn)
        rows = [dict(r) for r in await conn.fetch(
            "SELECT id, name, permissions, is_active, last_used, expires_at, created_at FROM api_tokens ORDER BY created_at DESC"
        )]
    return {"ok": True, "tokens": rows}


@router.post("/ip/block")
async def block_ip(payload: dict):
    hours = payload.get("hours", 24)
    blocked_until = datetime.utcnow() + timedelta(hours=hours)
    reason = payload.get("reason", "Manual block")
    async with get_conn() as conn:
        await _ensure_tables(conn)
        await conn.execute(_q("""
            INSERT INTO blocked_ips (ip_address, reason, blocked_until)
            VALUES (%s,%s,%s) ON CONFLICT (ip_address) DO UPDATE
            SET reason=%s, blocked_until=%s
        """), payload.get("ip"), reason, blocked_until, reason, blocked_until)
    return {"ok": True, "ip": payload.get("ip"), "blocked_until": blocked_until.isoformat()}


@router.get("/ip/blocked")
async def list_blocked():
    async with get_conn() as conn:
        await _ensure_tables(conn)
        rows = [dict(r) for r in await conn.fetch(
            "SELECT * FROM blocked_ips WHERE blocked_until > NOW() ORDER BY blocked_at DESC"
        )]
    return {"ok": True, "blocked_ips": rows}


@router.post("/event/log")
async def log_event(payload: dict):
    async with get_conn() as conn:
        await _ensure_tables(conn)
        await conn.execute(_q("""
            INSERT INTO security_events (event_type, ip_address, endpoint, details, severity)
            VALUES (%s,%s,%s,%s,%s)
        """), payload.get("event_type", "UNKNOWN"), payload.get("ip", "0.0.0.0"),
            payload.get("endpoint", "/"), payload.get("details", ""),
            payload.get("severity", "LOW"))
    return {"ok": True, "message": "Security event logged"}


@router.get("/events")
async def security_events():
    async with get_conn() as conn:
        await _ensure_tables(conn)
        rows = [dict(r) for r in await conn.fetch(
            "SELECT * FROM security_events ORDER BY created_at DESC LIMIT 50"
        )]
    return {"ok": True, "events": rows}


@router.get("/summary")
async def security_summary():
    async with get_conn() as conn:
        await _ensure_tables(conn)
        active_tokens = await conn.fetchval(
            "SELECT COUNT(*) FROM api_tokens WHERE is_active=TRUE"
        ) or 0
        blocked_ips = await conn.fetchval(
            "SELECT COUNT(*) FROM blocked_ips WHERE blocked_until > NOW()"
        ) or 0
        events_24h = await conn.fetchval(
            "SELECT COUNT(*) FROM security_events WHERE created_at > NOW() - INTERVAL '24 hours'"
        ) or 0
        severity_rows = await conn.fetch(
            "SELECT severity, COUNT(*) AS c FROM security_events GROUP BY severity"
        )
    by_severity = {r["severity"]: r["c"] for r in severity_rows}
    return {
        "ok": True,
        "active_tokens": active_tokens,
        "blocked_ips": blocked_ips,
        "events_last_24h": events_24h,
        "events_by_severity": by_severity,
        "security_status": "ALERT" if events_24h > 100 else "NORMAL",
    }
